...
function Container({ notes, updateNotes }) {
  ...
  function handleDrag(id) {
    console.log("hell");
    updateNotes((c) =>
      c.map((ci) => (ci.id === id ? { ...ci, dragging: !ci.dragging } : ci))
    );
  }
  ...
  return (
    <div className="added-notes">
      ...
      {notes.map((note, i) => (
        <p
          draggable="true"
          onDragStart={() => handleDrag(note.id)}
          onDragEnd={() => handleDrag(note.id)}
...
function Footer({ notes }) {
  ...
  return (
    <footer>
      You added {totalNotes} note(s), {activeNotes}{" "}
      {activeNotes <= 1 ? "is" : "are"} active and {inactiveNotes}{" "}
      {inactiveNotes <= 1 ? "is" : "are"} inactive
    </footer>
  );
}
